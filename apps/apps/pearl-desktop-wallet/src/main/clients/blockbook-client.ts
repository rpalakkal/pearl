...
async function fetchBlockbookJson<T>(network: BlockbookNetwork | undefined, path: string): Promise<T> {
...
async function postBlockbookText(network: BlockbookNetwork | undefined, path: string, body: string): Promise<string> {
    const response = await fetch(`${getBlockbookBaseUrl(network ?? getCurrentNetwork())}${path}`, {
        method: 'POST',
        headers: {
...
async function hydrateUtxoConfirmations(
    ...
    return Promise.all(utxos.map(async utxo => {
        ...
        try {
            ...
            return confirmations > (utxo.confirmations ?? 0)
                ? { ...utxo, confirmations }
                : utxo;
        } catch {
            return utxo;
