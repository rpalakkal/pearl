...
type Wallet struct {
	...
	// Guarded by backfillMtx; the map is created lazily. backfillRunMtx
	// serializes the scan goroutines themselves so only one backfill
	// batch-scans at a time.
	backfillMtx    sync.Mutex
	backfillRunMtx sync.Mutex
